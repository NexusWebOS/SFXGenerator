...
| **NightCode Net** *(NightCode)* | The terminal of [nightcode.coletechsystems.com](https://nightcode.coletechsystems.com): boots NightCode-DOS (BIOS POST, CONFIG.SYS, AUTOEXEC.BAT) and logs in to NightCode Net, a Supabase-backed bulletin board with accounts (sign up, activation email, username or email sign-in, password reset), message boards, WHO, FINGER, profiles, colour schemes and code rain. The same account works on the website and here; the program reads the site's `config.json`, so it follows whatever Supabase project the site uses. Run `nightcode`. Setup: [web/nightcode/SETUP.md](web/nightcode/SETUP.md). |
...
coleforge/
  ...
  web/nightcode/ nightcode.coletechsystems.com (Netlify) + its Supabase migration and login function
...
node coleforge/tests/nightcode.test.js    # NightCode Net's client against a mock Supabase, the migration's rules, the Netlify config step
