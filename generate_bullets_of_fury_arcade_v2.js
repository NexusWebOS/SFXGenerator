...
// Bullets of Fury arcade-quality V2 preview sound bank.
// Late-1990s arcade PCM direction: layered transients, filtered noise, body/sub,
// stereo diffusion and short-room coloration. No chip-wave presets, third-party
// samples, or game-file replacement.
...
const OUT = path.join(ROOT, "generated", "bullets-of-fury-arcade-v2");
...
function wave(kind, phase) {
  ...
  // Additive, band-limited-style bodies replace the raw discontinuous Atari/NES
  // waves from V1. They retain arcade bite without exposing 1-bit edges.
  if (kind === "triangle") {
    let v = 0;
    for (let h = 1; h <= 11; h += 2) v += Math.sin(Math.PI * 2 * p * h) * (h % 4 === 1 ? 1 : -1) / (h * h);
    return v * 0.82;
  }
  if (kind === "saw") {
    let v = 0;
    for (let h = 1; h <= 10; h++) v += Math.sin(Math.PI * 2 * p * h) / h;
    return v * 0.56;
  }
  if (kind === "square") {
    let v = 0;
    for (let h = 1; h <= 11; h += 2) v += Math.sin(Math.PI * 2 * p * h) / h;
    return v * 0.74;
  }
  return 0;
}
...
function tone(seconds, o = {}) {
  ...
  let phase = o.phase ?? 0, phaseR = phase + 0.173;
  for (let i = 0; i < s.l.length; i++) {
    const t = i / SR, q = t / s.seconds;
    const f = Math.max(18, f0 * Math.pow(Math.max(0.001, f1 / f0), q));
    phase += f / SR; phaseR += f * (o.detuneRatio ?? 1.0037) / SR;
    const vibrato = o.vib ? Math.sin(Math.PI * 2 * (o.vib.hz || 6) * t) * (o.vib.depth || 0.02) : 0;
    let v = wave(o.wave || "sine", phase + vibrato);
    let vr = wave(o.wave || "sine", phaseR - vibrato * 0.7);
    if (o.fm) {
      v = Math.sin(Math.PI * 2 * phase + Math.sin(Math.PI * 2 * phase * (o.fm.ratio || 2)) * (o.fm.depth || 2));
      vr = Math.sin(Math.PI * 2 * phaseR + Math.sin(Math.PI * 2 * phaseR * (o.fm.ratio || 2) + 0.31) * (o.fm.depth || 2));
    }
    ...
    s.l[i] = (v * (1 - width) + vr * width) * a * gl;
    s.r[i] = (vr * (1 - width) + v * width) * a * gr;
  }
  return s;
}
...
function noise(seconds, o = {}) {
  ...
  for (let i = 0; i < s.l.length; i++) {
    ...
    b0L = 0.99765 * b0L + rawL * 0.099046; b1L = 0.963 * b1L + rawL * 0.2965164; b2L = 0.57 * b2L + rawL * 1.0526913;
    b0R = 0.99765 * b0R + rawR * 0.099046; b1R = 0.963 * b1R + rawR * 0.2965164; b2R = 0.57 * b2R + rawR * 1.0526913;
    const pinkL = (b0L + b1L + b2L + rawL * 0.1848) * 0.19;
    const pinkR = (b0R + b1R + b2R + rawR * 0.1848) * 0.19;
    const coloredL = rawL * 0.38 + pinkL * 0.62, coloredR = rawR * 0.38 + pinkR * 0.62;
    lpL += alpha * (coloredL - lpL); lpR += alpha * (coloredR - lpR);
    const hpCut = o.hp ?? 0;
    let vL = lpL, vR = lpR;
    if (hpCut > 0) {
...
function circularDelay(dst, delaySec, gain, cross = 0) {
  const n = dst.l.length, d = Math.max(1, Math.round(delaySec * SR)) % n;
  const l0 = new Float32Array(dst.l), r0 = new Float32Array(dst.r);
  for (let i = 0; i < n; i++) {
    const j = (i - d + n) % n;
    dst.l[i] += (l0[j] * (1 - cross) + r0[j] * cross) * gain;
    dst.r[i] += (r0[j] * (1 - cross) + l0[j] * cross) * gain;
  }
  return dst;
}

function arcadeMaster(s, loop) {
  // Early reflections make the bank read like sampled arcade PCM in a cabinet,
  // not naked oscillator output. Loop reflections wrap circularly to remain exact.
  if (loop) {
    circularDelay(s, 0.017, 0.07, 0.72);
    circularDelay(s, 0.033, 0.045, 0.55);
  } else {
    delayed(s, 0.014, 0.075, 0.72);
    delayed(s, 0.031, 0.05, 0.58);
    delayed(s, 0.057, 0.027, 0.66);
  }

  const l0 = new Float32Array(s.l), r0 = new Float32Array(s.r), n = s.l.length;
  let detector = 0;
  for (let i = 0; i < n; i++) {
    const prev = loop ? (i - 1 + n) % n : Math.max(0, i - 1);
    const next = loop ? (i + 1) % n : Math.min(n - 1, i + 1);
    // Gentle three-tap anti-harshness plus restrained edge detail.
    let l = l0[i] * 0.84 + (l0[prev] + l0[next]) * 0.08;
    let r = r0[i] * 0.84 + (r0[prev] + r0[next]) * 0.08;
    const mid = (l + r) * 0.5, side = (l - r) * 0.5 * 1.12;
    l = mid + side; r = mid - side;
    if (!loop) {
      const level = Math.max(Math.abs(l), Math.abs(r));
      detector = Math.max(level, detector * 0.99945);
      const over = Math.max(0, detector - 0.42);
      const gain = 1 / (1 + over * 1.7);
      l *= gain; r *= gain;
    }
    s.l[i] = l; s.r[i] = r;
  }
...
function main() {
  ...
  const manifest = { title: "Bullets of Fury — Arcade PCM V2 Preview Bank", version: 2, generatedAt: new Date().toISOString(), sampleRate: SR, channels: 2, format: "16-bit stereo PCM WAV", soundDirection: "Layered late-1990s arcade shooter PCM: filtered-noise transients, body/sub layers, stereo diffusion, cabinet-style short reflections, and restrained saturation. No exposed chip-wave presets.", note: "Preview-only. V1 and all Bullets of Fury game assets remain untouched.", files: [] };
  ...
  finalize(reel, false); fs.writeFileSync(path.join(OUT, "bullets_of_fury_arcade_v2_preview_reel.wav"), wavBuffer(reel, false));
